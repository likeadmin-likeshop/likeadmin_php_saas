...
class TenantFileService
{
     ...
     */
    public static function getFileUrl($tenant_id, string $uri = '', string $type = ''): string
    {
        ...
        if (!$default) {
            $default = TenantConfigService::get('storage', $tenant_id, 'default', 'local');
        ...
        if ($default === 'local') {
            ...
            if (!$storage) {
                $storage = TenantConfigService::get('storage', $default);
     ...
     */
    public static function setFileUrl($uri, $tenant_id)
    {
        $default = TenantConfigService::get('storage', $tenant_id, 'default', 'local');
        if ($default === 'local') {
            $domain = request()->domain();
