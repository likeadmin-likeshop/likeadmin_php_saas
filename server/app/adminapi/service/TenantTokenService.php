...
class TenantTokenService
{
    ...
    {
        $time = time();
        $adminSession = TenantAdminSession::where([['tenant_id', '=', $adminId], ['terminal', '=', $terminal]])->find();
        //获取token延长过期的时间
        $expireTime = $time + Config::get('project.tenant_token.expire_duration');
        $adminTokenCache = new TenantTokenCache();
    ...
    public static function overtimeToken($token)
    {
        ...
        $adminSession->update_time = $time;
        $adminSession->save();
        return (new TenantTokenCache())->setAdminInfo($adminSession->token);
    }
